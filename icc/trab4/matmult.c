...
int main (int argc, char *argv[]){
    ...
    LIKWID_MARKER_REGISTER("multMatPtrVet");
    LIKWID_MARKER_START("multMatPtrVet");
    multMatPtrVet (mPtr_1, vet, n, n, resPtr);
    LIKWID_MARKER_STOP("multMatPtrVet");

    LIKWID_MARKER_REGISTER("multMatRowVet");
    LIKWID_MARKER_START("multMatRowVet");
    multMatRowVet (mRow_1, vet, n, n, resRow);
    LIKWID_MARKER_STOP("multMatRowVet");

    LIKWID_MARKER_REGISTER("multMatMatPtr");
    LIKWID_MARKER_START("multMatMatPtr");
    multMatMatPtr (mPtr_1, mPtr_2, n, resMatPtr);
    LIKWID_MARKER_STOP("multMatMatPtr");

    LIKWID_MARKER_REGISTER("multMatMatRow");
    LIKWID_MARKER_START("multMatMatRow");
    multMatMatRow (mRow_1, mRow_2, n, resMatRow);
    LIKWID_MARKER_STOP("multMatMatRow");
      
/*
    LIKWID_MARKER_GET("multMatPtrVet", &nevents, events, &time, &count);
    printf("multMatPtrVet: %i %f %f %i\n",nevents, events[0], time, count);
    LIKWID_MARKER_GET("multMatRowVet", &nevents, events, &time, &count);
    printf("multMatRowVet: %i %f %f %i\n",nevents, events[0], time, count);
    LIKWID_MARKER_GET("multMatMatPtr", &nevents, events, &time, &count);
    printf("multMatMatPtr:%i %f %f %i\n",nevents, events[0], time, count);
    LIKWID_MARKER_GET("multMatMatRow", &nevents, events, &time, &count);
    printf("multMatMatRow:%i %f %f %i\n",nevents, events[0], time, count);*/
